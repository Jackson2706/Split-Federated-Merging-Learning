...
def main():
    ...
    for epoch in tqdm(range(config["epochs"])):
        ...
        for idx in idxs_users:
            ...
            local_train_loader = DataLoader(
                local_train_dataset, batch_size=config["local_bs"], shuffle=True
            )
            ...
            for _ in range(config["local_ep"]):
                ...
                if config["is_gpu"]:
                    local_gpu_usages.append(
                        torch.cuda.memory_allocated(device=device) / 1024**2
                    )
                ...
                for image, label in local_train_loader:
                    ...
                    comm_cost_dict["client_upload_smashed_MB"] += (
                        (activation.numel() + label.numel()) * 4 / (1024**2)
                    )
                    comm_cost_dict[
                        "cloud_download_grad_MB"
                    ] += estimate_gradient_size_MB(
                        main_server_model, activation.shape, device
                    )
        ...
        comm_cost_dict["client_model_download_MB"] += (
            get_weight_size_mb(client_model_abs.state_dict())
            * config["num_users"]
        )
        ...
        merge_model.load_weight(
            copy.deepcopy(client_model_abs.state_dict()),
            copy.deepcopy(main_server_model.state_dict()),
        )
        ...
        with torch.no_grad():
            eval_loader = DataLoader(
                valid_dataset, batch_size=config["local_bs"], shuffle=False
            )
        ...
        round_gpu_usages.append(
            np.mean(round_gpu_per_client) if config["is_gpu"] else 0
        )

        if (epoch + 1) % print_every == 0:
            print(
                f"Epoch {epoch+1}: Train Loss {training_loss[-1]:.4f}, Eval F1 {eval_f1:.4f}"
            )
        if eval_f1 > best_f1:
            best_f1 = eval_f1
        ...
        print(
            f"F1 Score: {eval_f1_scores[-1]:.4f}, Loss: {total_loss / total_samples:.4f}"
        )
    ...
    json_path = f"/home/jackson/Desktop/Split-Federated-Merging-Learning/classification/Figure/data/SplitFL_{config['dataset']}_iid:{config['iid']}_{config['model']}_{config['num_users']} users.json"
    os.makedirs(
