...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        cosine_distance = torch.nn.functional.cosine_similarity(
            output1, output2
        )
        loss_contrastive = torch.mean(
            (1 - label) * torch.pow(cosine_distance, 2)
            + (label)
            * torch.pow(torch.clamp(self.margin - cosine_distance, min=0.0), 2)
        )
...
class HierarchicalFL:
    ...
    def train_end_to_end(
        ...
        for epoch in tqdm(range(1, epochs + 1)):
            ...
            loss_list = []
            for x, y in zip(
                torch.split(all_X, self.args["local_bs"]),
                torch.split(all_Y, self.args["local_bs"]),
            ):
