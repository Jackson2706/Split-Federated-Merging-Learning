...
def _aggregate_prototypes_and_generate_data(input_outputs, num_samples_per_class, device):
    ...
    final_dists = torch.stack([torch.stack(merged[l]['d']).mean(0) for l in final_labels]).to(device)
...
class HierarchicalFL:
    ...
    def _client_ssl_extraction_phase(self, cid, loader, ssl_epochs):
        ...
        # 1. SSL Training - Sử dụng non_blocking=True
        model.train()
        for _ in range(ssl_epochs):
            for data, _ in loader:
                data = data.to(self.device, non_blocking=True)
                optimizer.zero_grad(set_to_none=True) # Tốt hơn zero_grad()
                
                with torch.amp.autocast(device_type=self.device.type):
                    v1, v2 = self.ssl_transforms(data), self.ssl_transforms(data)
                    loss = info_nce_loss_4d(model(v1), model(v2))
    ...
    def _edge_ssl_extraction_phase(self, eid, cids, client_outputs, ssl_epochs, syn_samples_per_class):
        ...
        for epoch in range(ssl_epochs):
            ...
            for features, _ in syn_loader_L1: # Chỉ lấy features, bỏ qua labels
                features = features.to(self.device) # Dòng 604 (giờ đã OK)
                
                with torch.amp.autocast(device_type='cuda', enabled=(self.device.type == 'cuda')):
                    # Sửa lỗi Logic: Dùng augmentation 4D (geometric)
                    view_1 = self.ssl_transforms_edge(features)
                    view_2 = self.ssl_transforms_edge(features)
                    
                    z1 = model(view_1) # z1 là 4D
                    z2 = model(view_2) # z2 là 4D
                    
                    # Sửa lỗi Logic: Dùng loss 4D
                    loss = info_nce_loss_4d(z1, z2)
