...
def main():
    ...
    with open(
        f'/home/jackson/Desktop/Split-Federated-Merging-Learning/Figure/data/{config["dataset"]}_ourv2_{config["num_users"]}_{config["epochs"]}_{config["local_ep"]}_{config["t1"]}_{config["t2"]}_output.json',
        "w",
    ) as f:
    ...
    plt.plot(
        [config["t2"] * i for i in range(len(train_loss))],
        train_loss,
        color="r",
    )
    ...
    plt.savefig(
        "./save/Oursv2_{}_{}_loss.png".format(
    ...
    plt.plot(
        [config["t2"] * i for i in range(len(train_accuracy))],
        train_accuracy,
        color="k",
    )
    ...
    plt.xlabel("Communication Rounds")
    plt.savefig(
        "./save/Oursv2_{}_{}_f1.png".format(config["dataset"], config["epochs"])
    )
    ...
    plt.xlabel("Communication Rounds")
    plt.savefig(
        "./save/Oursv2_{}_{}_training_time.png".format(
            config["dataset"], config["epochs"]
        )
    )
    ...
    plt.xlabel("Communication Rounds")
    plt.savefig(
        "./save/Oursv2_{}_{}_cpu_usage.png".format(
            config["dataset"], config["epochs"]
        )
    )
    ...
    plt.xlabel("Communication Rounds")
    plt.savefig(
        "./save/Oursv2_{}_{}_ram_usage.png".format(
            config["dataset"], config["epochs"]
        )
    )
    ...
    plt.xlabel("Communication Rounds")
    plt.savefig(
        "./save/Oursv2_{}_{}_gpu_ram_usage.png".format(
            config["dataset"], config["epochs"]
        )
    )
